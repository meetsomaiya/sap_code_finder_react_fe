...
import sapLogo from '../assets/SAP CODE FINDER 1.png';
...
function LoginForm() {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            // const response = await fetch(`${BASE_URL}/login.php`, {
                // const response = await fetch('http://localhost:9113/login', {
                    // const response = await fetch('http://localhost:9211/login', {
            ...
            console.log("Full Response Data:", responseData);
                ...
                if (responseData.IsActive) {
                    ...
                    if (isAdmin.exists) {
                        ...
                        document.cookie = `domainId=${domainId}; expires=; path=/`;
    ...
    localStorage.setItem('user-name', isUserResponse.name);
    ...
    const checkAdmin = async (domainId, name) => {
        try {
            ...
            // const response = await fetch(`http://localhost:9114/checkAdmin?domainId=${domainId}&name=${name}`);
            const response = await fetch(`${BASE_URL}checkAdmin?domainId=${domainId}&name=${name}`);
            if (response.ok) {
    ...
    const checkUser = async (domainId, name) => {
        try {
            ...
            // const response = await fetch(`http://localhost:9115/checkUser?domainId=${domainId}&name=${name}`);
            const response = await fetch(`${BASE_URL}checkUser?domainId=${domainId}&name=${name}`);
            if (response.ok) {
                const data = await response.json();
        ...
        <main className="relative flex justify-center items-center h-screen w-screen">
            <div className="absolute aspect-w-16 aspect-h-9 h-full w-full bg-slate-300 bg-cover z-negative-10 overflow-hidden filter">
                <video autoPlay loop muted className="object-cover h-full w-full">
                    <source src={`${IMAGE_BASE_URL}/${video_t}`} type="video/mp4" />
                </video>
            </div>
            ...
            <div className="w-full h-full flex flex-row items-center justify-center sm:justify-end sm:pr-32 z-10">
                ...
                <div className="logo flex flex-row gap-2" style={{marginRight:'10px'}}>
                    <div className="logo-image">
                        <img src={sapLogo} alt="SAP Code Finder Logo" className="sap-finder-logo" />
                    </div>
