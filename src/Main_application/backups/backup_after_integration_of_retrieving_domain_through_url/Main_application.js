...
export default function MainApplication() {
    ...
    useEffect(() => {
        ...
        // Make API call if domain_id is available
        if (extractedDomainId) {
          const apiUrl = `${BASE_URL}/api_for_auto_login.php`; // Use template literals to construct the full URL
          ...
          fetch(apiUrl, {
            ...
            .then(data => {
              ...
              // Handle API response and extract required fields
              const { empCode, userName, emailId } = data.data;
    
              // Assign retrieved fields to state variables
              setEmpCode(empCode);
            ...
            .catch(error => {
              ...
              // Redirect to /login if API response was not 200 OK
              // window.location.href = '/login';
            });
        } else {
          console.error('Domain ID not found in URL');
          // Redirect to /login if domain_id is not found in URL
          // window.location.href = '/login';
        }
