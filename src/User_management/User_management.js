...
const UserManagementPage = () => {
  ...
  const [isUpdateModalOpen, setIsUpdateModalOpen] = useState(false);
    ...
    const fetchUsers = async (query = '') => {
      try {
        const url = `${BASE_URL}search_users.php?q=${encodeURIComponent(query)}`;
  ...
  const handleSearch = async () => {
    try {
      const response = await fetch(`${BASE_URL}search_users.php?q=${encodeURIComponent(searchQuery)}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${BASE_URL}add_new_user.php`, {
        method: 'POST',
        headers: {
      ...
      <div className='user_container'>
        ...
        />
         <button onClick={() => {}}>Search</button>
          ...
          <tbody>
            {(searchQuery ? searchResults : allUsers).map(user => (
